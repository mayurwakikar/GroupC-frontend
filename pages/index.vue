<template>
  <div>
    <h1 class="font-extrabold text-center text-5xl mt-8">
    ...
    <div>
      <div>
        <div class="text-center">
          <form
            method="post"
            class="flex justify-center bg-blue-100 px-12"
            autocomplete="off"
          >
            <table>
              <hr />
              <h2 class="text-teal-900 text-xl font-bold pt-6 text-center">
                Add a Book To Libaray
              </h2>
              <hr />
              <br />
              <label class="pt-10 py-10 mt-4" for="id">Book Id :- </label>
              <input
                class="border-black rounded-md border-2"
                type="number"
                id="id"
                name="id"
                placeholder=""
              ...
              <input
                ...
                @change="validationAuthor"
              /><br />
              <span
                class="text-sm text-red-500"
                v-for="error in v$.book_author.$errors"
                :key="error.$uid"
                >{{ error.$message }}</span
              >
              <br /><br />
              ...
              <span
                class="text-sm text-red-500"
                v-for="error in v$.book_price.$errors"
                :key="error.$uid"
                >{{ error.$message }}</span
              >
              <br /><br />
              ...
              <input
                ...
                name="book_image"
                placeholder=""
                v-model="formData.book_image"
                @change="validationImage"
              /><br />
              <span
                class="text-sm text-red-500"
              ...
              <input
                ...
                placeholder=""
                v-model="formDatacategory.bookcategory_id"
                @change="validationIsbn"
              /><br />
              <span
                class="text-sm text-red-500"
                v-for="error in vv$.bookcategory_id.$errors"
                :key="error.$uid"
                >{{ error.$message }}</span
              >
              <br /><br />
              ...
              <!-- <br/><br/> -->
              <select
                name="bookcategory_name"
                id="bookcategory_name"
                v-model="formDatacategory.bookcategory_name"
                multiple
              >
              ...
              <div class="mt-10">
                ...
                <button
                  class="py-1 px-2 mx-3 rounded-lg bg-green-600 hover:bg-green-600 text-white w-20"
                  @click="getalldata"
                >
                  All Books
                </button>
        ...
        <div class="border-black border-2 m-8 p-8 flex justify-center">
          <table class="list">
            <tr>
              <th class="px-4 border-black rounded-lg border-4">
                Book Category Id
              </th>
              <th class="px-4 border-black rounded-lg border-4">
                Book Category Name
            ...
            <tr
              v-for="itemcategory in statecategory.itemcategory"
              :key="itemcategory"
            >
              <td class="px-4 border-black rounded-lg border-4">
                {{ itemcategory.bookcategory_id }}
              </td>
              <td class="px-4 border-black rounded-lg border-4">
                {{ itemcategory.bookcategory_name }}
              </td>
            </tr>
          </table>
        </div>
...
async function getalldatacategory() {
  statecategory.itemcategory = await $fetch(
    "http://localhost:3003/bookcontroller"
  );
}
// 2nd part
